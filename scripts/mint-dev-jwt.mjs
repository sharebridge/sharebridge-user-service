 ...
 * Example: node scripts/mint-dev-jwt.mjs demo-user donor
 */
import "dotenv/config";
import { mintAuthToken } from "../src/tokenService.js";

const userId = process.argv[2]?.trim();
const role = process.argv[3]?.trim() || "donor";
if (!userId) {
  console.error("Usage: node scripts/mint-dev-jwt.mjs <user_id> [donor|coordinator]");
  process.exit(1);
}
