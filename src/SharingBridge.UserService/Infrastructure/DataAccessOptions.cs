...
public sealed class DataAccessOptions
{
    ...
    public const string RewritePortKey = "DB_SUPABASE_POOL_6543TRANS_5432SESSION";
    /// <summary>Previous env name — still accepted as a fallback.</summary>
    public const string RewritePortKeyLegacy = "DB_REWRITE_SUPABASE_TRANSACTION_PORT";
    ...
    /// <summary>
    /// When true, rewrite Supabase transaction pooler <c>:6543</c> → session <c>:5432</c>
    /// on <c>*.pooler.supabase.com</c> (Npgsql-safe).
    /// </summary>
    public bool SupabasePool6543TransTo5432Session { get; init; } = true;
    ...
    public static DataAccessOptions FromConfiguration(IConfiguration config)
    {
        return new DataAccessOptions
        {
            ...
            SupabasePool6543TransTo5432Session =
                TryReadBool(config, RewritePortKey)
                ?? TryReadBool(config, RewritePortKeyLegacy)
                ?? true,
    ...
    public object ToPublicConfig() => new
    {
        ...
        supabase_pool_6543trans_5432session = SupabasePool6543TransTo5432Session,
